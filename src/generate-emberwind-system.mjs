...
class SystemGenerator {
   ...
   * Rename files.
   *
   * Rename files that had emberwind in their name, such as
   * css/emberwind.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*emberwind*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('emberwind', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-emberwind-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Emberwind's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace emberwind name mentions in files.
      generator.replaceFileContents();
      // Rename files that had emberwind in their name.
